...
def image_to_video(buffer, bucket_id):
    ...
    print(f'Processing generation index: {bucket_id}')
...
def process_bucket_id(bucket_id, text_prompt, save_dir):
    try:
        image_buffer = text_to_image(text_prompt)
        video_generation_id = image_to_video(image_buffer, bucket_id)  
        video_content = check_video_status(video_generation_id)
        with open(os.path.join(save_dir, f"{bucket_id}_video.mp4"), 'wb') as file:
            file.write(video_content)
            print(f"Video saved to {os.path.join(save_dir, f'{bucket_id}_video.mp4')}")
...
def handle_message(text_prompt) -> None:
    ...
    save_dir = os.path.join(os.getcwd(), start_time) + f"_cfg_id_{cfg_scale}"
    print(f"Saving videos to {save_dir}")
    # Create the directory if it does not exist
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)

    for i in range(1, 256, 20):
        with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
            # Start a new thread for each bucket_id in the current batch
            executor.map(process_bucket_id, range(i, min(i+20, 256)), [text_prompt]*20, [save_dir]*20)
